...
import { FiBell, FiCheckCircle, FiAlertTriangle, FiInfo, FiTrash2, FiCheck } from 'react-icons/fi';
              ...
              {notifications.map((n) => {
                ...
                return (
                  <div key={n.id} className={`${styles.item} ${!n.read ? styles.unread : ''}`}>
                    <div className={`${styles.iconWrap} ${styles[TYPE_TONE[n.type]]}`}>
                      <Icon size={15} />
                    </div>
                    ...
                    <button className={styles.deleteBtn} onClick={() => deleteNotification(n.id)} aria-label="Delete notification">
                      <FiTrash2 size={14} />
                    </button>
                  </div>
                );
